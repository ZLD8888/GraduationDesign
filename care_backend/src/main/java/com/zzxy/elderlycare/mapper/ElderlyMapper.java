...
public interface ElderlyMapper {
    ...
    @Insert("insert into elderly_info(name, gender, birth_date, id_card, phone, bed_number, check_in_date, caregiver_id, health_condition, dietary_restrictions, emergency_contact_name, emergency_contact_phone, created_at, updated_at) " +
            "values(#{name}, #{gender}, #{birthDate}, #{idCard}, #{phone},#{bedNumber}, #{checkInDate}, #{caregiverId}, #{healthCondition}, #{dietaryRestrictions}, #{emergencyContactName}, #{emergencyContactPhone}, #{createdAt}, #{updatedAt})")
    void addElderly(Elderly elderly);

    @Insert("insert into users(phone, password, name, role, created_at, updated_at) " +
            "VALUES (#{phone}, '123456', #{name}, 'ELDERLY', #{createdAt}, #{updatedAt})")
    ...
    @Insert("insert into care.staff_elderly(staff_id, elderly_id) values(#{userId}, #{elderlyId})")
    void addElderlyAndUserId(Integer userId, String elderlyId);
